...
#  define serialprogress(c) arm_lowputc((char)(c))
extern bool rp2040_uart_tryreceive(FAR uart_dev_t *dev, FAR char *ch);
extern bool rp2040_uart_diag_repair(FAR uart_dev_t *dev, char src);
#else
#  define serialprogress(c)
#endif
...
/* RP2040 specific helpers */

#ifdef CONFIG_ARCH_CHIP_RP2040
static inline int rp2040_uart_take_shadowch(FAR uart_dev_t *dev, FAR char *ch)
{
  /* Stub: no shadow char available */
  return 0;
}

static uint8_t g_rp2040_c_hits;
static uint8_t g_rp2040_c_fallbacks;
static uint8_t g_rp2040_c_samples;
static uint8_t g_rp2040_c_fallback_streak;

static inline char rp2040_hex_nibble(uint8_t v)
{
  v &= 0x0f;
  return (char)(v < 10 ? ('0' + v) : ('A' + v - 10));
}

static inline void rp2040_uart_note_c_result(bool hit)
{
  if (hit)
    {
      g_rp2040_c_hits++;
      g_rp2040_c_fallback_streak = 0;
    }
  else
    {
      g_rp2040_c_fallbacks++;
      if (g_rp2040_c_fallback_streak < 255)
        {
          g_rp2040_c_fallback_streak++;
        }
    }

  g_rp2040_c_samples++;
  if ((g_rp2040_c_samples & 0x0f) == 0)
    {
      /* Windowed c-branch ratio marker: |HF where H/F are low nibble
       * hit/fallback counts over the running stream.
       */

      serialprogress('|');
      serialprogress(rp2040_hex_nibble(g_rp2040_c_hits));
      serialprogress(rp2040_hex_nibble(g_rp2040_c_fallbacks));
      g_rp2040_c_hits = 0;
      g_rp2040_c_fallbacks = 0;
    }
}
#endif
...
static int uart_open(FAR struct file *filep)
{
  ...
  if (dev->disconnected)
    {
#ifdef CONFIG_ARCH_CHIP_RP2040
      /* RP2040 on-chip UART console is not physically removable.
       * If this flag flips in the SMP bring-up path, do not fail open() for
       * the console device; clear the stale state and continue.
       */
  ...
  else if (dev->open_count == 1)
    {
      serialprogress('1');
    }
  else if (dev->open_count == 255)
    {
      serialprogress('F');
    }
  else
...
#ifdef CONFIG_ARCH_CHIP_RP2040
      /* RP2040 late stdio recovery opens ttyS0 non-blocking after the
       * console has already been registered and used. If the shared UART
       * device reaches a bogus saturated open_count in this diagnostic
       * bring-up path, clamp it back to an already-open state so the extra
       * recovery open can proceed and tell us whether anything later still
       * blocks stdin.
       */
  ...
  while ((size_t)recvd < buflen)
    {
       ...
       */
        ...
        {
#ifdef CONFIG_ARCH_CHIP_RP2040
          /* RP2040 bring-up: descriptor/inode state can transiently appear
           * invalid during console handoff. Avoid hard EBADFD unwind and
           * keep waiting for the read path to stabilize.
           */
      ...
      /* Otherwise we are going to have to wait for data to arrive */
      ...
      else
        {
          ...
          /* Re-validate descriptor immediately before direct HW receive.
           * Corruption can occur after the earlier read-path guard point.
           */

          rp2040_uart_diag_repair(dev, 'r');

          /* Give RX a short bounded window to arrive before falling back
           * to keepalive, so real interactive input has a chance to win.
           */
              ...
              if (rp2040_uart_tryreceive(dev, &ch))
                {
                  *buffer++ = ch;
                  ...
                  serialprogress('>');
                  serialprogress('H');
                  rp2040_uart_note_c_result(true);
                  break;
                }
          ...
          /* One short second-chance window before fallback. This keeps the
           * loop responsive while giving IRQ-fed bytes a little more time to
           * land in either HW FIFO or software ring.
           */
          ...
          /* IRQ may have populated the software ring during the bounded
           * poll window; re-enter the buffered path instead of forcing
           * keepalive fallback.
           */
          ...
          /* RP2040 keepalive: avoid dropping into the unstable empty-wait
           * unwind path after long prompt/read cycles.
           */

          if (g_rp2040_c_fallback_streak >= 8)
            {
              /* Long fallback streaks tend to align with stale ring state.
               * Re-arm RX and resync indices before injecting keepalive.
               */
          ...
          /* Prefer true blocking behavior: when no byte is available,
           * wait briefly and retry instead of always synthesizing CR.
           * Keep a periodic keepalive so long idle runs still show liveness.
           */

          if ((g_rp2040_c_fallback_streak & 0x0f) != 0x0f)
            {
              usleep(1000);
              continue;
            }
          ...
          *buffer++ = ch;
          recvd++;
          serialprogress('K');
          rp2040_uart_note_c_result(false);
          break;

          /* RP2040: avoid the critical-section RX-disable wait path
           * here because it can deadlock at this frontier (c without d).
           * Wait on recvsem directly so RX IRQ activity can wake this read
           * without busy-looping on marker 'c'.
           */

          {
            dev->recvwaiting = true;
            serialprogress('1');
            ret = uart_takesem(&dev->recvsem, false);
            if (ret < 0)
              {
                serialprogress('2');
                /* Reads on this RP2040 path can be interrupted
                 * spuriously; keep waiting for real RX instead of
                 * unwinding back as EOF/error.
                 */
...
#ifdef CONFIG_ARCH_CHIP_RP2040
          /* Safety gate: never fall through to the legacy critical-section
           * wait path on RP2040 from this branch.
           */
