/**
 * OpenFC2040 Basic Firmware
 * A simple firmware for the OpenFC2040 flight controller
 * Enhanced with LED debugging and interactive commands
 */
...
// LED pin alternatives for testing
uint led_pins_red[] = {2, 25, 15, 14};    // Common red LED pins
uint led_pins_green[] = {3, 24, 16, 13};  // Common green LED pins  
uint led_pins_blue[] = {4, 23, 17, 12};   // Common blue LED pins
uint current_led_set = 0;
bool led_active_low = true;
bool manual_led_mode = false;  // When true, disable heartbeat LED
...
int main() {
    ...
    while (true) {
        ...
        // Heartbeat LED (blue) every 1 second - only if not in manual mode
        if (!manual_led_mode && (loop_count - last_heartbeat >= 100)) {  // 100 * 10ms = 1 second
            led_set_rgb(false, false, true);
            sleep_ms(50);
            led_set_rgb(false, false, false);
        ...
        if (loop_count % 1000 == 0) {
            printf("\nOpenFC2040 running... Loop: %lu (LED mode: %s)\n", 
                   loop_count, manual_led_mode ? "Manual" : "Auto");
            printf("> ");
        }
        
        sleep_ms(10);  // 10ms loop
        loop_count++;
    }
